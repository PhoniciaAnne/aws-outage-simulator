...
function stepOutage() {
  ...
  setIncidentBanner(true, `🚨  BILLING SUBGRAPH INCIDENT — ${name} Service Disruption · us-east-1`);
  startIncidentTimer();
  dtMode = 'outage';
  ...
  setText('rh-status-msg', `${name} tier disruption — Billing subgraph affected · Failover engaging`);
...
function stepLambdaInvoked() {
  ...
  addLog('🟣', 'Lambda: Performing regional failover logic for the Billing Subgraph', 'recovery');
...
function stepTrafficShifted() {
  addLog('🎯', 'FAILOVER COMPLETE — 100% of Billing API traffic routed to us-west-2', 'success');
  addLog('✅', 'Secondary Region Active — All subgraph services healthy in us-west-2', 'success');
...
function stepRecovered() {
  ...
  addLog('🏁', `RTO ACHIEVED: ${rtoFmt} · Target was 5 min · Subgraph operational`, 'success');
...
function resetSim() {
  ...
  setTextEl('replication-label', 'DYNAMODB GLOBAL REPLICATION', '#1e3050');
  ...
  setText('rh-status-icon', '✅');
  setText('rh-status-msg', 'All disruption types within RTO/RPO targets. Global resiliency configured.');
  ...
  addLog('✅', 'System reset — Billing Subgraph fully operational in us-east-1', 'success');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  Object.entries(ddbRegionInit).forEach(([r, v]) => setDdbRegion(r, v.cls, v.text));
  updateHealthBadge();
  addLog('🟢', 'AWS DR Simulator ready — Click "RANDOM Outage" or select a service manually.', 'system');
});
